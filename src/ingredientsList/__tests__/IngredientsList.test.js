...
// "setup" uses param (input = {}) to create a proxy "input" Object,
// which is used by arrow func to assign properties to the "setup" function object
const setup = (input = {}) => ({
...
describe("Ingredients List", () => {
  ...
  it("makes no IngredientsListItems if no ingredients are passed in", () => {
    const testEnv = setup({ makeIngredientsUndefined: true });
    const wrapper = shallow(<IngredientsList {...testEnv} />);

    expect(getElement(wrapper)("div")("ingredients-list-container").children("IngredientsListItem").length).toBe(0);
  });

  it("makes no IngredientsListItems if an empty ingredients list is passed in", () => {
    const testEnv = setup({ ingredients: [] });
    const wrapper = shallow(<Ingredients {...testEnv} />);
  ...
  it("passes the nextIngredient prop to the value of an AddTextItemInput component", () => {
    const testEnv = setup({ nextIngredient: `I'm Batman` });
    const wrapper = shallow(<IngredientsList {...testEnv} />);
  ...
  it("passes the addIngredient callback to the addHandler of a AddTextItemInput component", () => {
    const testEnv = setup({ addIngredient: jest.fn() });
    const wrapper = shallow(<IngredientsList {...testEnv} />);
  ...
  it("passes the handleAddNextChange callback to the changeHandler of an AddTextItemInput component", () => {
    const testEnv = setup({ handleAddNextChange: jest.fn() });
    const wrapper = shallow(<IngredientsList {...testEnv} />);

    expect(wrapper.find("AddTextItemInput").props().changeHandler).toBe(testEnv.handleAddNextChange);
  });

  it("disables the add button on the AddTextItemInput component if canSearch is false and we have ingredients in the list", () => {
    const testEnv = setup({
      canSearch: false,
      ingredients: [`I'm Batman`]
    });
    const wrapper = shallow(<IngredientList {...testEnv} />);
  ...
  it("enables the add button on the AddTextItemInput component if canSearch is false but we have no ingredients", () => {
    // canSearch will be false if we have no ingredients, but we need to be able to add at least one, that's what we're testing here
    const testEnv = setup({
      canSearch: false,
      ingredients: []
    });
