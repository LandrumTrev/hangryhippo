  ...
  render() {
    return (
      <div className="recipe-item-list">
        {(!this.props.items || this.props.items.length === 0) && "No recipes found"}
        {this.props.items && this.props.items.length > 0 && this.props.items.map((item, index) => <RecipeItem {...item} showRecipe={this.props.showRecipe} key={index} />)}
      </div>
    );
  }
}
