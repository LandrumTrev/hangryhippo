...
describe("Recipe List Item", () => {
  ...
  it("should show the number of ingredients used", () => {
    ...
    const wrapper = shallow(<RecipeItem usedIngredientsCount={testUsedIngredientsCount} />);

    expect(
      getElement(wrapper)("section")("recipe-list-item-used-ingredients")
        .text() // in the text content of div tag
        .indexOf(`${testUsedIngredientsCount}`) // the value of the var
    ).not.toEqual("-1"); // should not be absent
  ...
  it("should say ALL ingredients used instead of a number, if all ingredients are used", () => {
    ...
    const wrapper = shallow(<RecipeItem usedIngredientsCount={testUsedIngredientsCount} missedIngredientCount={testMissingIngredientsCount} />);
